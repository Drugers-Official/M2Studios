          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                ...
                className={`nav-link text-foreground hover:text-primary transition-all duration-300 ${pathname === link.href ? "active" : ""}`}
              >
                {link.label}
              </Link>
            ))}
            ...
            <button
              ...
              className="px-4 py-2 text-white font-medium border border-white rounded-full transition-all duration-300 hover:bg-white hover:text-black hover:shadow-[0_0_10px_rgba(255,255,255,0.3)] active:shadow-[0_0_15px_rgba(255,255,255,0.4)]"
            >
          ...
          <div className="md:hidden py-4 space-y-4" style={{ borderTop: "1px solid rgba(250,204,21,0.2)" }}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`nav-link block text-foreground transition-all duration-300 ${pathname === link.href ? "active" : ""}`}
                onClick={handleNavClick}
              >
                {link.label}
              </Link>
            ...
            <button
              onClick={handleLogout}
              className="w-full px-4 py-2 text-white font-medium border border-white rounded-full transition-all duration-300 hover:bg-white hover:text-black"
            >
              <LogOut size={18} className="inline mr-2" />
              Logout
            </button>
